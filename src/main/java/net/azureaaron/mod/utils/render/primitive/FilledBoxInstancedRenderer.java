...
import net.azureaaron.mod.utils.render.BoxDataUniform;
...
public final class FilledBoxInstancedRenderer implements AutoCloseable {
	...
	public void submitPrimitives(List<FilledBoxRenderState> states, CameraRenderState cameraState) {
		...
		if (!normalStates.isEmpty()) {
			...
			VertexConsumer normalBuffer = Renderer.getBuffer(AaronModRenderPipelines.FILLED_INSTANCED, TextureSetup.noTexture(), 1f, normalStates.size(), normalUniform);
		...
		if (!throughWallsStates.isEmpty()) {
			...
			VertexConsumer throughWallsBuffer = Renderer.getBuffer(AaronModRenderPipelines.FILLED_THROUGH_WALLS_INSTANCED, TextureSetup.noTexture(), 1f, throughWallsStates.size(), throughWallsUniform);
