...
public final class FilledBoxInstancedRenderer {
	...
	public void submitPrimitives(List<FilledBoxRenderState> states, CameraRenderState cameraState) {
		...
		if (!normalStates.isEmpty()) {
			...
			BufferBuilder normalBuffer = Renderer.getBuffer(AaronModRenderPipelines.FILLED_INSTANCED, TextureSetup.noTexture(), 1f, normalStates.size(), normalUniform);
		...
		if (!throughWallsStates.isEmpty()) {
			...
			BufferBuilder throughWallsBuffer = Renderer.getBuffer(AaronModRenderPipelines.FILLED_THROUGH_WALLS_INSTANCED, TextureSetup.noTexture(), 1f, throughWallsStates.size(), throughWallsUniform);
