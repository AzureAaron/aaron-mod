...
public class GlowRenderer implements AutoCloseable {
	...
	private void tryUpdateDepthTexture() {
		...
		if (this.glowDepthTexture == null || this.glowDepthTexture.getWidth(0) != neededWidth || this.glowDepthTexture.getHeight(0) != neededHeight) {
			...
			this.glowDepthTexture = device.createTexture(() -> "Aaron Mod Glow Depth Tex", GpuTexture.USAGE_RENDER_ATTACHMENT | GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING, TextureFormat.DEPTH32, neededWidth, neededHeight, 1, 1);
