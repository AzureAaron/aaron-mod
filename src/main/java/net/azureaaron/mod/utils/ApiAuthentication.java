	...
	private static void updateToken() {
		...
		((MinecraftClientAccessor) CLIENT).getProfileKeyPairManager().prepareKeyPair().thenAcceptAsync(playerKeypairOpt -> {
			...
			if (playerKeypairOpt.isPresent() && !expired) {
				...
				TokenRequest.KeyPairInfo keyPairInfo = new TokenRequest.KeyPairInfo(Objects.requireNonNull(CLIENT.getUser().getProfileId()), publicKey, publicKeySignature, expiresAt);
				...
				try {
					...
					CLIENT.schedule(() -> Scheduler.INSTANCE.schedule(ApiAuthentication::updateToken, refreshAtTicks, true));
				} catch (Exception e) {
					//Try again in 15 minutes
					logErrorAndScheduleRetry(Component.literal(AUTH_FAILURE), 900 * 20, "[Aaron's Mod Api Auth] Failed to refresh the api token! Some features might not work :(", e);
				}
			} else {
				...
				logErrorAndScheduleRetry(Component.literal(NO_PROFILE_KEYS), expired ? -1 : 300 * 20, "[Aaron's Mod Api Auth] Failed to fetch profile keys! Some features may not work temporarily :( (Has your game been open for more than 24 hours? If so restart.)");
			}
		}).exceptionally(throwable -> {
			//Try again in 5 minutes
			logErrorAndScheduleRetry(Component.literal(AUTH_FAILURE), 300 * 20, "[Aaron's Mod Api Auth] Encountered an unexpected exception while refreshing the api token!", throwable);
	...
	private record TokenRequest(KeyPairInfo keyPairInfo, SignedData signedData, String mod, String minecraftVersion, String modVersion) {
		...
		private record KeyPairInfo(UUID uuid, String publicKey, byte[] publicKeySignature, long expiresAt) {
			private static final Codec<KeyPairInfo> CODEC = RecordCodecBuilder.create(instance -> instance.group(
					UUIDUtil.STRING_CODEC.fieldOf("uuid").forGetter(KeyPairInfo::uuid),
					Codec.STRING.fieldOf("publicKey").forGetter(KeyPairInfo::publicKey),
					ExtraCodecs.BASE64_STRING.fieldOf("publicKeySignature").forGetter(KeyPairInfo::publicKeySignature),
		...
		private record SignedData(byte[] original, byte[] signed) {
			private static final Codec<SignedData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
					ExtraCodecs.BASE64_STRING.fieldOf("original").forGetter(SignedData::original),
					ExtraCodecs.BASE64_STRING.fieldOf("signed").forGetter(SignedData::signed))
