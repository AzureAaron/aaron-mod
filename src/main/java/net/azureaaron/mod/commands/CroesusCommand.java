...
public class CroesusCommand {
	...
	private static int handleCommand(FabricClientCommandSource source, CommandPlayerData playerData) {
		...
		.thenAccept(profileData -> {
			if (profileData != null) {
				try {
					printCroesus(source, profileData, playerData.name(), playerData.uuid());
				} catch (Throwable t) {
	...
	private static void printCroesus(FabricClientCommandSource source, JsonObject body, String name, String uuid) {
