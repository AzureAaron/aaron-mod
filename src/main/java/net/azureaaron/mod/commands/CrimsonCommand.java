...
public class CrimsonCommand {
	...
	private static int handleCommand(FabricClientCommandSource source, CommandPlayerData playerData) {
		...
		.thenAccept(profileData -> {
			if (profileData != null) {
				try {
					printCrimson(source, profileData, playerData.name(), playerData.uuid());
				} catch (Throwable t) {
	...
	private static void printCrimson(FabricClientCommandSource source, JsonObject body, String name, String uuid) {
